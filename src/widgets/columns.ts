...
export type ColumnWidth =
  ...
  | `${number}%`; // Percentage of available width
 ...
 */
function isWidget(obj: unknown): obj is Widget {
  return (
    typeof obj === "object" &&
    obj !== null &&
    "render" in obj &&
    typeof (obj as Widget).render === "function"
  );
}
 ...
 */
function getColumnContent(col: ColumnDef, ctx: RenderContext): string[] {
...
function calculateColumnWidths(
  ...
  const widths: (number | null)[] = new Array(columns.length).fill(null);
  let flexCount = 0;
  let percentTotal = 0;
  ...
  if (flexCount > 0) {
    const flexWidth = Math.max(
      minWidth,
      Math.floor(remainingWidth / flexCount),
    );
    let flexAssigned = 0;
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      if (width === "flex") {
        ...
        if (flexAssigned === flexCount) {
          widths[i] = Math.max(
            minWidth,
            remainingWidth - flexWidth * (flexCount - 1),
          );
        } else {
          widths[i] = flexWidth;
        }
 ...
 */
function fitText(text: string, width: number, alignment: Alignment): string {
 ...
 */
class ColumnsWidget implements Widget {
  readonly type = "columns" as const;

  constructor(private options: ColumnsOptions) {}
  ...
  render(ctx: RenderContext): string[] {
    ...
    const normalizedColumns = columns.map((col) =>
      normalizeColumn(col, defaultWidth),
    );

    // Render content for each column
    const contents: string[][] = normalizedColumns.map((col) =>
      getColumnContent(col, ctx),
    );
...
export function Columns(
  optionsOrColumns: ColumnsOptions | (ColumnDef | string | Widget)[],
): Widget {
