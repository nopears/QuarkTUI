...
// Rendering
// =============================================================================
function renderMultiSelectMenu(config, focusedIndex, checkedIndices) {
    ...
    const availableContentLines = height -
        headerLineCount -
        footerLineCount -
        infoLineCount -
    ...
    drawLine(`${pad}${theme.colors.textMuted}${selectionHint}${RESET}`, innerWidth);
...
export async function multiSelect(options) {
    ...
    while (true) {
        ...
        // Navigation
        if (isUpKey(key)) {
            focusedIndex = findNextEnabled(focusedIndex > 0 ? focusedIndex : maxIndex + 1, -1);
            renderMultiSelectMenu(config, focusedIndex, checkedIndices);
        }
        else if (isDownKey(key)) {
            focusedIndex = findNextEnabled(focusedIndex < maxIndex ? focusedIndex : -1, 1);
            renderMultiSelectMenu(config, focusedIndex, checkedIndices);
        }
 ...
 * ```
 */
export async function quickMultiSelect(title, items, initiallyChecked = []) {
