...
export function createKeyboardHandler(
  ...
  let cleanedUp = false;
  const handlers: Set<(key: KeypressEvent) => void | Promise<void>> = new Set();
  ...
  const internalHandler = (str: string | undefined, key: readline.Key) => {
...
export function waitForKeypressCancellable(): {
  ...
  let keyboard: KeyboardHandler | null = null;

  const promise = new Promise<KeypressEvent>((resolve) => {
 ...
 */
export function isBackKey(key: KeypressEvent): boolean {
  return (
    key.name === "backspace" ||
    key.name === "escape" ||
    key.str === "q" ||
 ...
 */
export function isConfirmKey(key: KeypressEvent): boolean {
 ...
 */
export function isUpKey(key: KeypressEvent): boolean {
 ...
 */
export function isDownKey(key: KeypressEvent): boolean {
 ...
 */
export function getNumberKey(key: KeypressEvent): number | null {
 ...
 */
export function isPrintable(key: KeypressEvent): boolean {
